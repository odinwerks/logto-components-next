  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      ...
      <div style={styles.noteStyle}>
        ...
        If <code style={styles.codeSmStyle}>BACKEND_TYPE</code> is completely unset, there are two client-side <code style={styles.codeSmStyle}>readEnv</code> calls with different defaults. The session-heartbeat component (<code style={styles.codeSmStyle}>session-heartbeat.tsx:30</code>) defaults to <code style={styles.codeSmStyle}>upstream</code>, matching the server and safely skipping pings. The sessions tab (<code style={styles.codeSmStyle}>tabs/sessions.tsx:243</code>) defaults to <code style={styles.codeSmStyle}>blacktop</code>, which is a mismatch: it shows stale <code style={styles.codeSmStyle}>lastActiveAt</code> values even though the server is running in upstream mode with heartbeats disabled. The three-way default when <code style={styles.codeSmStyle}>BACKEND_TYPE</code> is unset is: server=<code style={styles.codeSmStyle}>upstream</code>, client-heartbeat=<code style={styles.codeSmStyle}>upstream</code> (safe), client-sessions=<code style={styles.codeSmStyle}>blacktop</code> (mismatch). Pass <code style={styles.codeSmStyle}>BACKEND_TYPE</code> as both a runtime env var and <code style={styles.codeSmStyle}>NEXT_PUBLIC_BACKEND_TYPE</code> build arg to keep server and client in sync.
      </div>
