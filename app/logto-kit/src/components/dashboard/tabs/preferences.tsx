  ...
  const handleSave = async () => {
    try {
      ...
      if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
        setParseError(t.preferences.mustBeObject);
        return;
      }

      setIsLoading(true);
      await onUpdateCustomData(parsed);
      onSuccess(t.preferences.success);
      setIsEditing(false);
      setParseError('');
      refreshData();
    } catch (error) {
      if (error instanceof SyntaxError) {
        setParseError(`${t.preferences.invalidJson}: ${error.message}`);
      } else {
        onError(error instanceof Error ? error.message : t.preferences.error);
      }
    } finally {
      setIsLoading(false);
    }
        ...
        {isEditing ? (
          <div>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              ...
              >
                {isLoading ? t.common.loading : t.preferences.save}
              </button>
              <button
